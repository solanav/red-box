...
def test_fetch(IMAP4):
    ...
    assert (
        msg.html_body
        == """<div dir="ltr">Hi,<div><br></div><div>this is an example.</div><div><br></div><div>- Mikael</div></div>\n"""
    )
    ...
    assert msg.date == datetime.datetime(
        2022, 10, 1, 0, 23, 4, tzinfo=datetime.timezone(datetime.timedelta(hours=3))
    )
